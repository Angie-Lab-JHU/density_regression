...
class Dropout:
	def __init__(self, model, opts, dataset="", learning_rate=1e-3, tag=""):
		...
		self.loss_function = libs.losses.MSE if self.mse else libs.losses.Gaussian_NLL
	...
	def run_train_step(self, x, y):
		with tf.GradientTape() as tape:
			...
			if self.mse:
				...
				mu, logsigma = tf.split(y_hat, 2, axis=-1)
				loss = self.loss_function(y, mu, tf.exp(logsigma))
	...
	def evaluate(self, x, y):
		...
		if self.mse:
			...
			mean_mu, mean_sigma = tf.split(mu, 2, axis=1)
			loss = self.loss_function(y, mean_mu, mean_sigma)
			
		rmse = libs.losses.RMSE(y, mean_mu)

		tau = self.l**2 * (1-self.drop_prob) / (2. * self.lam) # https://www.cs.ox.ac.uk/people/yarin.gal/website/blog_3d801aa532c1ce.html
		var += tau**-1
		nll = libs.losses.Gaussian_NLL(y, mean_mu, tf.sqrt(var))
	...
	def save_train_summary(self, loss, x, y, y_hat):
		with self.train_summary_writer.as_default():
			...
			tf.summary.scalar('mse', tf.reduce_mean(libs.losses.MSE(y, y_hat)), step=self.iter)
	...
	def save_val_summary(self, loss, x, y, mu, var):
		with self.val_summary_writer.as_default():
			...
			tf.summary.scalar('mse', tf.reduce_mean(libs.losses.MSE(y, mu)), step=self.iter)
			idx = np.random.choice(int(tf.shape(x)[0]), 9)
			if tf.shape(x).shape==4:
				tf.summary.image("x", [gallery(tf.gather(x,idx).numpy())], max_outputs=1, step=self.iter)
